...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const updated = await readAndWrite('wallet', (entries) => {
        const entry = {
          id: generateId(),
          type: body.type || 'in', // 'in' or 'out'
          personOrPurpose: body.personOrPurpose || '',
    ...
    if (req.method === 'DELETE') {
      await readAndWrite('wallet', (entries) => entries.filter(e => e.id !== body.id));
